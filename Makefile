...
install-python:
	wget https://www.python.org/ftp/python/3.8.2/Python-3.8.2.tgz -P ~/python38
	cd ~/python38 && tar zxfv Python-3.8.2.tgz && rm Python-3.8.2.tgz && mv Python-3.8.2 python
	cd ~/python38/python && ./configure --prefix=~/python38 --enable-optimizations && make && make install
